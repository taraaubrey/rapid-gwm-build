...
class MF6Simulation(Simulation, SimulationType):
    ...
    _default_file = (
        r"src\rapid_gwm_build\mf6_template.yaml",
    ...
    def __init__(
        ...
        super().__init__(
            ws, name, model_type, cfg, _defaults
    ...
    def build(self):  # TODO: maybe rename to write?
        ...
        gwfs = {}
        gwf_packages = [
            m for m in module_registry.values() if m.kind == "gwf"
        ]  # TODO if
        ...
        for gwf_pkg in gwf_packages:
            ...
            if dis.model_name:  # can specify in the cfg if a package is associated with a specific model
                raise ValueError(
                    "Model name in dis package does not match gwf package."
                )
